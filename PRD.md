...
# DataPersistence PoC — 개발 Phase 계획
...
비즈니스 로직(상태 전이, 생산 스케줄링 등)은 이 PoC의 범위 밖.

## Phase 구성

각 Phase는 Red → Green → Refactor 순서로 진행하며, 각 단계마다 사용자 승인 후 다음 단계로 진행.  
Phase 완료 시 Git push.
...
**테스트 대상:**
- `save(sample)` → 저장 후 반환
- `find_by_id(id)` → 존재/비존재
- `find_all()` → 전체 목록 / 빈 목록
- `update(id, data)` → 수정 / 미존재 시 None
- `delete(id)` → 삭제 성공/실패
- 파일 재생성 후 데이터 영속성 확인
...
**테스트 대상:**
- Sample과 동일한 CRUD
- 상태(`status`)별 조회 (`find_by_status`)
- 유효하지 않은 상태값 저장 시 예외 처리
